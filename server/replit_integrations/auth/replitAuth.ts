...
// Session user type for passport
...
export const isAuthenticated: RequestHandler = async (req, res, next) => {
  ...
  // Hydrate full user record from database (including companyId, role, etc.)
  try {
    const userId = sessionUser.claims?.sub;
    if (userId) {
      const dbUser = await authStorage.getUser(userId);
      if (dbUser) {
        // Attach full user record to request, keeping claims for compatibility
        (req as any).user = {
          ...sessionUser,
          ...dbUser,
    ...
    // Continue with session user if DB lookup fails
  }
